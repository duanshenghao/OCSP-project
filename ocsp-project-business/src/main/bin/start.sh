...
# 打印启动命令
STARTUP_LOG="${STARTUP_LOG}application background startup command: nohup java ${JAVA_OPT} -Xbootclasspath/a:${CONFIG_DIR} -jar ${BASE_PATH}/boot/${APPLICATION_JAR} > ${LOG_PATH} 2>&1 &\n"
...
echo begin start ...
nohup java ${JAVA_OPT} -DNACOS_NAMESPACE=${NACOS_NAMESPACE} -DNACOS_ADDRESS=${NACOS_ADDRESS} -Xbootclasspath/a:${CONFIG_DIR} -jar ${BASE_PATH}/boot/${APPLICATION_JAR} >${LOG_PATH} 2>&1 &
echo end start ...
